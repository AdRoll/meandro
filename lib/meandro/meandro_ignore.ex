defmodule Meandro.Ignore do
  ...
  def ignores(files_and_asts) do
    for {file, module_asts} <- files_and_asts,
      ...
      {file, ignores}
    end |> List.foldl(%{}, fn ({key, value}, map) -> Map.update(map, key, value, fn existing_value -> List.flatten([value | existing_value]) end) end)
  end
  ...
  defp result_ignored?(%Meandro.Rule{file: file, rule: rule, pattern: found_pattern}, %Meandro.Ignore{
         file: file,
         rule: rule,
